...
{
    handle->index++;
    handle->extra->moins = ft_atoi(handle->str + handle->index);
...
{
    handle->index++;
    handle->extra->blanck = ft_atoi(handle->str + handle->index);
...
{
    handle->index++;
    handle->extra->precision = ft_atoi(handle->str + handle->index);
...
{
    handle->index++;
    handle->extra->zero = ft_atoi(handle->str + handle->index);
    handle->index += ft_nbrlen((unsigned long long)handle->extra->zero, 0);
}

void        ft_set_width(t_printf *handle)
{
    handle->extra->width = ft_atoi(handle->str + handle->index);
